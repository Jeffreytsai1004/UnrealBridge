...
| M4-4 | `SaveMeshToNewStaticMesh(handle, NewAssetPath, MaterialList: TArray<UMaterialInterface*>) -> FString` | 包 `CreateNewStaticMeshAssetFromMesh`；走 `AssetTools::CreateAsset`（不要直接 `NewObject`+`Package`，参考 `enhanced-input-roadmap.md` 坑 #8） |
